...
(function($) {
	...
	$.fn.FancyFileUpload = function(options) {
		...
		var AddFile = function(uploads, e, data) {
			var inforow = $('<div class="ff_fileupload_preview col-6 col-md-4 col-xl-3 mt-4 mx-auto"><input type="hidden" class="ff_file_data_info_drop" input_name="'+data.paramName+'" style="display:none;"><button class="ff_fileupload_preview_image" type="button"><span class="ff_fileupload_preview_text"></span></button><div class="ff_fileupload_actions_mobile"></div></td><td class="ff_fileupload_actions"></div>');
			...
			if (URL && URL.createObjectURL)
			{
				...
				if (url)
				{
					...
					let files_new = data.files[0];
					let reader = new FileReader();
					reader.onload = function(event) {
						inforow.find('.ff_file_data_info_drop').attr('file_data', event.target.result);
					}
					reader.readAsDataURL(files_new);
					inforow.find('.ff_file_data_info_drop').attr('file_name', data.files[0].name);
				}
			}
			...
			if($('#'+data.paramName).prop('multiple') == false){

				uploads.html(inforow);
			}else{
		...
		return this.each(function() {
			...
			var dropzone = $('<button>').addClass('ff_fileupload_dropzone').attr('type', 'button').attr('aria-label', Translate('Browse files here to attach'));
			...
			var uploads = $('<div>').addClass('ff_fileupload_uploads scroll-inner fix-width order-last');
